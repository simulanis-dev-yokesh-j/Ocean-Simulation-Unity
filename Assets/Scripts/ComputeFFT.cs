...
public static class ComputeFFT
{
    ...
    static ComputeFFT()
    {
        ...
        for (int i = 0; i < _supportedSizes.Length; i++)
        {
            ...
            _sizeErrorMessage += _supportedSizes[i] + ((i < _supportedSizes.Length - 1) ? ", " : ".");
        }
    }
    ...
    public static void IFFT2D(CommandBuffer cmd, RenderTexture input, bool scale = true, bool permute = false)
    {
        DoFFT(cmd, input, true, scale, permute);
    }

    private static void DoFFT(CommandBuffer cmd, RenderTexture input, bool inverse, bool scale, bool permute)
    {
